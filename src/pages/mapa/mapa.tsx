...
const categoryMap: { [key: string]: string } = {
  ...
  "entertainment.culture": "Cultura",
  "entertainment.museum": "Museus",
  "commercial.shopping_mall": "Shoppings",
  "commercial.marketplace": "Mercados / Feiras",
  "accommodation": "Hospedagem",
  "beach": "Praias",
  "natural": "Natureza",
  "tourism.sights.place_of_worship": "Igrejas",
  "heritage": "Patrimônios Históricos",
...
export function Mapa() {
  ...
  const { categorySlug } = useParams<{ categorySlug: string }>(); // 2. Leia o slug da categoria da URL
  ...
  useEffect(() => {
      ...
      if (preferences && userLocation && allPlaces.length > 0) {
          ...
          const activityToCategories: { [key: string]: string[] } = {
              ...
              'praia': ['beach'] // Adicione esta se tiver a categoria
          };
  ...
  // 3. NOVO useEffect: Filtra os locais com base na CATEGORIA da URL
  useEffect(() => {
    // Só executa se tivermos um slug de categoria e a lista de locais
    if (categorySlug && allPlaces.length > 0) {
      // Mapeamento do slug da URL para as categorias da API
      const slugToCategories: { [key: string]: string[] } = {
        'restaurantes-e-bares': ['catering.restaurant', 'catering.fast_food', 'catering.bar'],
        'praias-e-natureza': ['beach', 'natural'],
        'hospedagem': ['accommodation'],
        'cultura-e-historia': ['entertainment.culture', 'tourism.attraction'],
        'lazer-e-entretenimento': ['leisure', 'entertainment'],
        'compras-e-artesanato': ['commercial.shopping_mall', 'commercial.marketplace'],
      };

      const categoryFilter = slugToCategories[categorySlug];

      if (categoryFilter) {
        const filteredByCategory = allPlaces.filter(place =>
          place.properties.categories.some(cat => categoryFilter.includes(cat))
        );
        
        // Agrupa os locais filtrados e atualiza o estado
        const grouped: GroupedPlaces = {};
        filteredByCategory.forEach(place => {
          const mainCategory = place.properties.categories.find(c => categoryMap[c]) || 'outros';
          const categoryName = categoryMap[mainCategory] || 'Outros';
        ...
        setPlaces(grouped);
        setNotification(`Exibindo locais da categoria: ${categorySlug.replace(/-/g, ' ')}`);
      } else {
        // Se o slug não for encontrado, mostra todos
        setPlaces(groupPlaces(allPlaces));
      }
      // Limpa qualquer local selecionado anteriormente
      setSelectedPlace(null);
    } else if (!preferences) {
        // Se não veio nem do modal nem da categoria, mostra tudo
        setPlaces(groupPlaces(allPlaces));
    }
  }, [categorySlug, allPlaces]); // Dependências do efeito
...
// Certifique-se de que a função groupPlaces esteja acessível aqui
function groupPlaces(places: Place[]): GroupedPlaces {
    // Agrupa os locais por categoria principal
    const grouped: GroupedPlaces = {};
